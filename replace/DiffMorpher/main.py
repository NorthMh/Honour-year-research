...
for idx, (path0, path1, cap0, cap1) in enumerate(zip(image_path[::2], image_path[1::2], prompts[::2], prompts[1::2])):
    ...
    print("type:", type(input_seva_latents))
    print("type:", type(mid_latents))
    torch.save(input_seva_latents, f"{output_folder}/lora_1_{save_name}.pt")
    torch.save(mid_latents, f"{output_folder}/lora_2_{save_name}.pt")
