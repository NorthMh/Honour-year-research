...
class DiffMorpherPipeline(StableDiffusionPipeline):
    ...
    @torch.no_grad()
    def cal_latent(self, num_inference_steps, guidance_scale, unconditioning, img_noise_0, img_noise_1, text_embeddings_0, text_embeddings_1, lora_0, lora_1, alpha, use_lora, fix_lora=None, lora=None):
        ...
        if use_lora:
            if fix_lora is not None:
                ...
                self.unet = load_lora(self.unet, lora)
            else:
    ...
    def __call__(
        ...
        name_0 = img_path_0.split("/")[-1].split(".")[0]
        name_1 = img_path_1.split("/")[-1].split(".")[0]

        lora_list = []
        if augmented_img_list is not None:
            for i in range(len(augmented_img_list)):
                lora_list.append(Image.open(augmented_img_list[i]).convert("RGB"))
        
        if self.use_lora:
            print("Loading lora...")
            if augmented_img_list is not None:
                weight_name = f"{output_path.split('/')[-1]}_lora_{name_0}_{name_1}.ckpt"
                load_lora_path = save_lora_dir + "/" + weight_name
                if not os.path.exists(load_lora_path):
                    train_lora(lora_list, prompt_0,prompt_1, search_0, search_1, train_batch_size, save_lora_dir, None, self.tokenizer, self.text_encoder,
                               self.vae, self.unet, self.scheduler, lora_steps, lora_lr, lora_rank, weight_name=weight_name)
                    
            print(f"Load from {load_lora_path}.")
            if load_lora_path.endswith(".safetensors"):
                lora = safetensors.torch.load_file(
                    load_lora_path, device="cpu")
            else:
                lora = torch.load(load_lora_path, map_location="cpu")    
        else:
            lora = None
        ...
        if if_test_pca:
            input_seva_latents = []
            search_lora_list = []
            img1_lora_list = lora_list[0:len(lora_list)//2]
            img2_lora_list = lora_list[len(lora_list)//2:]
            for i in range(search_0+1):
                search_lora_list.append(img1_lora_list[i])
            for i in range(search_1+1):
                search_lora_list.append(img2_lora_list[i])
            
            for img in search_lora_list:
                input_seva_latents.append(self.image2latent(get_img(img)))
            input_seva_latents = torch.cat(input_seva_latents, dim=0) # (N, 4, 64, 64)
        ...
        with torch.no_grad():
            if self.use_reschedule:
                ...
                if if_test_pca:
                    mid_images = images[1:-1]
                    mid_latents = []
                    for img in mid_images:
                        mid_latents.append(self.image2latent(get_img(img)))
                    mid_latents = torch.cat(mid_latents, dim=0)
            else:
                alpha_list = list(torch.linspace(0, 1, num_frames))
                print(alpha_list)
        ...
        if if_test_pca:
            print("input_seva_latents device:", input_seva_latents.device)
            print("mid_latents device:", mid_latents.device)
            return images, input_seva_latents, mid_latents
        else:
            return images
